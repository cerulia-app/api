...
export function createCampaignService(runtime: ServiceRuntime) {
  return {
    async create(
      ...
      if (input.houseRef) {
        const house = await requireRecord<{ defaultRuleProfileRefs?: string[] }>(
          runtime,
          input.houseRef,
          COLLECTIONS.house,
          'houseRef',
    ...
    async update(
      ...
      if (record.repoDid !== callerDid) {
        return rejected('forbidden-owner-mismatch', 'campaignRef must belong to the caller')
      }
      ...
      if (
        ...
        return rejected('terminal-state-readonly', 'archived campaigns are read-only')
      }

      const nextHouseRef = input.houseRef ?? record.value.houseRef
      if (nextHouseRef) {
        const house = await requireRecord(runtime, nextHouseRef, COLLECTIONS.house, 'houseRef')
        if (house.repoDid !== callerDid) {
          return rejected('forbidden-owner-mismatch', 'houseRef must belong to the caller')
        }
      }
      ...
      const nextRuleProfiles = input.sharedRuleProfileRefs ?? record.value.sharedRuleProfileRefs
      const refsError = await validateRuleProfileRefs(
        runtime,
        callerDid,
        nextRuleProfiles,
    ...
    async getView(
      ...
      const ruleOverlay = await Promise.all(
        (record.value.sharedRuleProfileRefs ?? []).map((ref) =>
          requireRecord<AppCeruliaCoreRuleProfile.Main>(runtime, ref, COLLECTIONS.ruleProfile, 'ruleProfileRef'),
        ),
      )
