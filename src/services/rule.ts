...
export function createRuleService(runtime: ServiceRuntime) {
  return {
    ...
    async createProfile(
      ...
      if (!collection) {
        return rejected('invalid-required-field', 'scopeKind must be house-shared or campaign-shared')
      }

      if (parseAtUri(input.scopeRef).repoDid !== callerDid) {
        return rejected('forbidden-owner-mismatch', 'scopeRef must belong to the caller')
      }

      await requireRecord(runtime, input.scopeRef, collection, 'scopeRef')

      const uriError = assertCredentialFreeUri(input.rulesPatchUri, 'rulesPatchUri')
      if (uriError) {
    ...
    async updateProfile(
      ...
      if (record.repoDid !== callerDid) {
        return rejected('forbidden-owner-mismatch', 'ruleProfileRef must belong to the caller')
      }
    ...
    async listProfiles(
      ...
      const records = await runtime.store.listRecords<AppCeruliaCoreRuleProfile.Main>(
        COLLECTIONS.ruleProfile,
        callerDid,
      )
