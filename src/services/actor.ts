  ...
  return {
    async updateProfile(
      ...
      if (input.blueskyProfileRef) {
        ...
        if (parsed.repoDid !== callerDid || parsed.collection !== COLLECTIONS.blueskyProfile) {
          return rejected('forbidden-owner-mismatch', 'blueskyProfileRef must point to the caller profile')
        }
      }
      ...
      if (!(await blobBelongsToCaller(runtime, callerDid, input.avatarOverrideBlob))) {
        return rejected('invalid-required-field', 'avatarOverrideBlob must belong to the caller repo')
      }

      if (!(await blobBelongsToCaller(runtime, callerDid, input.bannerOverrideBlob))) {
        return rejected('invalid-required-field', 'bannerOverrideBlob must belong to the caller repo')
      }

      const profileRef = `at://${callerDid}/${COLLECTIONS.playerProfile}/${SELF_RKEY}`
      const existing = await runtime.store.getRecord<AppCeruliaCorePlayerProfile.Main>(profileRef)
      const createdAt = existing?.createdAt ?? runtime.now()
      const updatedAt = runtime.now()
      const record = {
        $type: COLLECTIONS.playerProfile,
        ownerDid: callerDid,
        blueskyProfileRef: input.blueskyProfileRef ?? existing?.value.blueskyProfileRef,
        displayNameOverride: input.displayNameOverride ?? existing?.value.displayNameOverride,
        descriptionOverride: input.descriptionOverride ?? existing?.value.descriptionOverride,
        avatarOverrideBlob: input.avatarOverrideBlob ?? existing?.value.avatarOverrideBlob,
        bannerOverrideBlob: input.bannerOverrideBlob ?? existing?.value.bannerOverrideBlob,
        websiteOverride: input.websiteOverride ?? existing?.value.websiteOverride,
        pronounsOverride: input.pronounsOverride ?? existing?.value.pronounsOverride,
        roleDistribution: input.roleDistribution ?? existing?.value.roleDistribution,
        ...
        playableTimeSummary: input.playableTimeSummary ?? existing?.value.playableTimeSummary,
        preferredScenarioStyles:
          input.preferredScenarioStyles ?? existing?.value.preferredScenarioStyles,
    ...
    async getProfileView(
      ...
      const profileRecord = await runtime.store.getRecord<AppCeruliaCorePlayerProfile.Main>(profileRef)
      ...
      const publicBranches = await Promise.all(
        branches
          ...
          .map(async (branch): Promise<AppCeruliaActorGetProfileView.BranchLink> => {
            const sheet = await requireRecord<AppCeruliaCoreCharacterSheet.Main>(
              runtime,
      ...
      return {
        profileSummary: composeProfileSummary(did, profileRecord?.value, fallback),
        publicBranches,
      }
    },
  }
