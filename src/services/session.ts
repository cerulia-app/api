  ...
  return {
    async create(
      ...
      if (hasScenarioRef === hasScenarioLabel) {
        return rejected('invalid-exactly-one', 'scenarioRef and scenarioLabel must be exactly one')
      }

      if (input.role === 'pl' && !input.characterBranchRef) {
        return rejected('invalid-required-field', 'role=pl requires characterBranchRef')
      }

      if (input.characterBranchRef && !hasSameOwner(input.characterBranchRef, callerDid)) {
        return rejected('forbidden-owner-mismatch', 'characterBranchRef must belong to the caller')
      }

      if (input.scenarioRef) {
        await requireRecord(runtime, input.scenarioRef, COLLECTIONS.scenario, 'scenarioRef')
      }

      if (input.characterBranchRef) {
        await requireRecord(runtime, input.characterBranchRef, COLLECTIONS.characterBranch, 'characterBranchRef')
      }

      if (input.campaignRef) {
        await requireRecord(runtime, input.campaignRef, COLLECTIONS.campaign, 'campaignRef')
      }
    ...
    async update(
      ...
      const nextScenarioRef = input.scenarioRef ?? (input.scenarioLabel ? undefined : record.value.scenarioRef)
      const nextScenarioLabel = input.scenarioLabel ?? (input.scenarioRef ? undefined : record.value.scenarioLabel)
      const nextRole = input.role ?? record.value.role
      const nextCharacterBranchRef = input.characterBranchRef ?? record.value.characterBranchRef

      if (Boolean(nextScenarioRef) === Boolean(nextScenarioLabel)) {
        return rejected('invalid-exactly-one', 'updated session must include exactly one scenario identity')
      }
      ...
      if (nextCharacterBranchRef && !hasSameOwner(nextCharacterBranchRef, callerDid)) {
        return rejected('forbidden-owner-mismatch', 'characterBranchRef must belong to the caller')
      }

      if (nextScenarioRef) {
        await requireRecord(runtime, nextScenarioRef, COLLECTIONS.scenario, 'scenarioRef')
      }

      if (nextCharacterBranchRef) {
        await requireRecord(runtime, nextCharacterBranchRef, COLLECTIONS.characterBranch, 'characterBranchRef')
      }
      ...
      const nextRecord = {
        ...
        externalArchiveUris: input.externalArchiveUris ?? record.value.externalArchiveUris,
    ...
    async list(
      ...
      const records = await runtime.store.listRecords<AppCeruliaCoreSession.Main>(
        COLLECTIONS.session,
        callerDid,
      )
      ...
      const items = await Promise.all(
        page.items.map(async (record): Promise<AppCeruliaSessionList.SessionListItem> => ({
