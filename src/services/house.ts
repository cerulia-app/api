...
export function createHouseService(runtime: ServiceRuntime) {
  return {
    async create(
      callerDid: string,
      input: AppCeruliaHouseCreate.InputSchema,
    ) {
    ...
    async update(
      callerDid: string,
      input: AppCeruliaHouseUpdate.InputSchema,
    ) {
      ...
      if (record.repoDid !== callerDid) {
        return rejected('forbidden-owner-mismatch', 'houseRef must belong to the caller')
      }
      ...
      const nextRuleProfiles = input.defaultRuleProfileRefs ?? record.value.defaultRuleProfileRefs
      const refsError = await validateDefaultRuleProfiles(runtime, callerDid, nextRuleProfiles)
      if (refsError) {
        return rejected('invalid-schema-link', refsError)
      }
      ...
      const nextRecord = {
        ...
        externalCommunityUri: input.externalCommunityUri ?? record.value.externalCommunityUri,
    ...
    async getView(
      ...
      const campaigns = (await runtime.store.listRecords<AppCeruliaCoreCampaign.Main>(
      ...
      const sessions = (await runtime.store.listRecords<AppCeruliaCoreSession.Main>(
        COLLECTIONS.session,
      )).filter((session) => session.value.campaignRef && campaignRefs.has(session.value.campaignRef))
