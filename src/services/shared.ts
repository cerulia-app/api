...
export async function requireRecord<T>(
  ...
  if (parsed.collection !== collection) {
    throw new ApiError('InvalidRequest', `${label} must reference ${collection}`, 400)
  }
...
export function assertCredentialFreeUri(
  ...
  return isCredentialFreeUri(value) ? null : `${label} must be a credential-free public URI`
}
...
export async function loadBlueskyProfile(
  ...
  const profileRef = explicitRef ?? `at://${did}/${COLLECTIONS.blueskyProfile}/${SELF_RKEY}`
