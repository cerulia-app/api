...
export function createCharacterService(runtime: ServiceRuntime) {
  async function resolvedBranchStats(
    ...
    if (branch.overridePayload) {
      current = mergeJsonObject(current, branch.overridePayload) as { [_ in string]: unknown }
    }
    ...
    for (const advancement of orderedAdvancements) {
      current = mergeJsonObject(current, advancement.value.deltaPayload) as { [_ in string]: unknown }
    }
  ...
  return {
    async createSheet(
      ...
      if (schema.value.baseRulesetNsid !== input.rulesetNsid) {
        return rejected('invalid-schema-link', 'sheetSchemaRef baseRulesetNsid must match rulesetNsid')
      }

      if (input.stats !== undefined) {
        const err = validateStatsAgainstSchema(input.stats, schema.value.fieldDefs)
        if (err) {
          return rejected('invalid-required-field', err)
        }
      }

      if (!(await blobBelongsToCaller(runtime, callerDid, input.portraitBlob))) {
        return rejected('invalid-required-field', 'portraitBlob must belong to the caller repo')
      }
    ...
    async updateSheet(
      ...
      if (record.repoDid !== callerDid) {
        return rejected('forbidden-owner-mismatch', 'characterSheetRef must belong to the caller')
      }
    ...
    async rebaseSheet(
      ...
      if (schema.value.baseRulesetNsid !== record.value.rulesetNsid) {
        return rejected('invalid-schema-link', 'targetSheetSchemaRef must match the sheet ruleset')
      }

      const nextStats = input.stats ?? record.value.stats
      if (nextStats !== undefined) {
        const err = validateStatsAgainstSchema(nextStats, schema.value.fieldDefs)
        if (err) {
          return input.stats
    ...
    async createBranch(
      ...
      if (sheet.repoDid !== callerDid) {
        return rejected('forbidden-owner-mismatch', 'baseSheetRef must belong to the caller')
      }
    ...
    async updateBranch(
      ...
      if (record.repoDid !== callerDid) {
        return rejected('forbidden-owner-mismatch', 'characterBranchRef must belong to the caller')
      }

      if (record.value.retiredAt) {
        return rejected('terminal-state-readonly', 'retired branches are read-only')
      }
    ...
    async recordAdvancement(
      ...
      if (
        ...
        return rejected('invalid-required-field', 'previousValues is required for retrain/respec/correction')
      }

      if (input.sessionRef) {
        if (!hasSameOwner(input.sessionRef, callerDid)) {
          return rejected('forbidden-owner-mismatch', 'sessionRef must belong to the caller')
        }

        await requireRecord(runtime, input.sessionRef, COLLECTIONS.session, 'sessionRef')
      }
    ...
    async recordConversion(
      ...
      const sourceBranch = await requireRecord<AppCeruliaCoreCharacterBranch.Main>(
        runtime,
        input.sourceBranchRef,
        COLLECTIONS.characterBranch,
        'sourceBranchRef',
      )
      const targetBranch = await requireRecord<AppCeruliaCoreCharacterBranch.Main>(
        runtime,
      ...
      if (
        ...
        return rejected('forbidden-owner-mismatch', 'all conversion refs must belong to the caller')
      }
      ...
      if (
        ...
        return rejected('invalid-schema-link', 'rulesetNsid values must match the referenced sheets')
      }
      ...
      if (
        ...
        return rejected('invalid-schema-link', 'source/target branch refs must match the referenced sheets')
      }
    ...
    async getHome(callerDid: string): Promise<AppCeruliaCharacterGetHome.OutputSchema> {
      const branches = await runtime.store.listRecords<AppCeruliaCoreCharacterBranch.Main>(
        COLLECTIONS.characterBranch,
        callerDid,
      )
      const sessions = await runtime.store.listRecords<AppCeruliaCoreSession.Main>(
        COLLECTIONS.session,
        callerDid,
      )
      ...
      const recentSessions = await Promise.all(
        [...sessions]
          .sort((left, right) => right.value.playedAt.localeCompare(left.value.playedAt))
          .map(async (record) =>
    ...
    async getBranchView(
      ...
      const advancements = (await runtime.store.listRecords<AppCeruliaCoreCharacterAdvancement.Main>(
        COLLECTIONS.characterAdvancement,
        branch.repoDid,
      )).filter((record) => record.value.characterBranchRef === branchRef)
      const conversions = (await runtime.store.listRecords<AppCeruliaCoreCharacterConversion.Main>(
        COLLECTIONS.characterConversion,
        branch.repoDid,
      )).filter((record) => {
        return record.value.sourceBranchRef === branchRef || record.value.targetBranchRef === branchRef
      })
      const sessions = (await runtime.store.listRecords<AppCeruliaCoreSession.Main>(
        COLLECTIONS.session,
        branch.repoDid,
        ...
        advancementSummaries: await Promise.all(
          advancements.map(async (record) => ({
            ...
            sessionSummary: record.value.sessionRef
              ? await (async () => {
                  const session = sessions.find((sessionRecord) => sessionRecord.uri === record.value.sessionRef)
                  if (!session) {
                    return undefined
                  }
