...
function isExtensibleGroup(field: FieldDef): boolean {
  return field.fieldType === 'group' && 'extensible' in field && field.extensible === true
}
...
function validateField(
  ...
  if (value === undefined) {
    return options.partial || !field.required
      ? null
      : `${path} is required`
  }
...
export function validateStatsAgainstSchema(
  ...
  for (const field of fieldDefs) {
    const err = validateField(stats[field.fieldId], field, field.fieldId, options)
    if (err) {
      return err
    }
  }
...
function flattenField(
  ...
  switch (field.fieldType) {
    case 'group': {
      ...
      const childMap = new Map((field.children ?? []).map((child) => [child.fieldId, child]))
      for (const [key, nestedValue] of Object.entries(value)) {
    ...
    case 'array': {
      ...
      value.forEach((entry, index) => {
        flattenField(field.itemDef as FieldDef, entry, `${path}[${index}]`, entries)
      })
