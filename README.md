...
親 workspace に sibling の `../protocol` repo がある場合、`bun install` 後の postinstall で自動的に local protocol package へ張り替える。単独 clone では declared package dependency のまま動く。
